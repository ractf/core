...
class HintTestCase(ChallengeSetupMixin, APITestCase):
    ...
    def test_hint_post(self):
        self.client.force_authenticate(self.user)
        response = self.client.post(reverse('hint-list'),
                                    data={'name': 'test-hint', 'penalty': 100, 'challenge': self.challenge2.id})
        self.assertEquals(response.status_code, HTTP_403_FORBIDDEN)

    def test_hint_detail_put(self):
        self.client.force_authenticate(self.user)
        response = self.client.put(reverse('hint-detail', kwargs={'pk': self.hint1.id}), data={'name': 'test-hint'})
        self.assertEquals(response.status_code, HTTP_403_FORBIDDEN)
    ...
    def test_hint_detail_patch_admin(self):
        ...
        self.user.save()
        self.client.force_authenticate(self.user)
        response = self.client.patch(reverse('hint-detail', kwargs={'pk': self.hint3.id}), data={'name': 'test-hint'})
        self.assertEquals(response.status_code, HTTP_200_OK)
