...
def send_email(send_to, subject_line, template_name, **template_details):
    if settings.MAIL["SEND"]:  # pragma: no cover
        if settings.MAIL["SEND_MODE"] == "AWS":  # pragma: no cover
            ...
            data = {
                "personalizations": [{"to": [{"email": send_to}], "subject": subject_line}],
                "from": {"email": settings.MAIL["SEND_ADDRESS"], "name": settings.MAIL["SEND_NAME"]},
                "content": [
                    {"type": "text/plain", "value": render_to_string(template_name + ".txt", template_details)},
                    {"type": "text/html", "value": render_to_string(template_name + ".html", template_details)},
                ],
            }
