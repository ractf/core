...
class ChallengeSerializerMixin:
    ...
    def get_votes(self, instance):
        return {"positive": instance.votes_positive, "negative": instance.votes_negative}
...
class CategorySerializer(serializers.ModelSerializer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        solves = user.team.solves if user.team else user.solves
        self.fields["challenges"].context.update(
            {
                "request": self.context["request"],
                "solves": list(solves.filter(correct=True).values_list("challenge", flat=True)),
            }
        )
