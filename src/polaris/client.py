...
def handle_request(method, path, **kwargs):
    response = requests.request(method, f"{settings.POLARIS_URL}/{path}",
                                auth=HTTPBasicAuth(settings.POLARIS_USERNAME, settings.POLARIS_PASSWORD), **kwargs)
...
def allocate_instance(challenge_id, user):
    response = post('/instanceallocation', json={
...
def reallocate_instance(challenge_id, user):
    response = post('/instanceallocation/new', json={
        'challenge': str(challenge_id),
        'user': str(user.id),
        'team': str(user.team.id)
    })
