...
class RecalculateUserViewTestCase(APITestCase):
    ...
    def setUp(self):
        ...
        user.save()
        admin_user = get_user_model()(username='recalculate-test-admin', email='recalculate-test-admin@example.org')
    ...
    def test_recalculate(self):
        ...
        self.assertEquals(get_user_model().objects.get(id=self.user.id).points, total + 100)
    ...
    def test_recalculate_leaderboard(self):
        ...
        self.client.force_authenticate(self.admin_user)
        self.client.post(reverse('recalculate-user', kwargs={'id': self.user.id}))
        self.assertEquals(get_user_model().objects.get(id=self.user.id).leaderboard_points, total)
